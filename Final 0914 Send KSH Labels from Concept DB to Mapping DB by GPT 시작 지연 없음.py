# -*- coding: utf-8 -*-
"""
파일명: 0914 Send KSH Labels from Concept DB to Mapping DB by GPT 시작 지연 없음.py
Version: 2.1.0
업데이트 일시: 2025-09-14 (최근 업데이트)

KSH 레이블 매핑 스크립트 (성능 최적화 버전)
...
[2025-09-14 업데이트 내역 - v2.1.0]
- _extract_korean_only 함수 개선: ksh_korean 컬럼 공백 완전 제거
- _extract_ksh_codes_from_text 함수 최적화: 정규화된 ksh 컬럼 형식에 맞춘 패턴 개선
- 인덱스 활용 극대화를 위한 검색 로직 최적화
- 쉼표 구분자 기반 KSH 코드 정확 매칭 지원
- 레거시 패턴 호환성 유지하면서 성능 개선
...
class KSHLabelMapper:
    ...
    def _extract_ksh_codes_from_text(self, text: str) -> List[str]:
        ...
        ※ 2025-08-31 업데이트: 0831 파일의 KSH 정규화 이후 형식에 맞춤
        - 기존: "nlk:KSH1998029903; nlk:KSH1998005028"
        - 신규: "KSH1998039283, KSH1998009488, KSH2000016107"
        ...
        # -------------------
        # ✅ 수정: 정규화된 형식에 맞는 패턴만 사용
        # 쉼표 또는 세미콜론으로 구분된 KSH 코드들을 추출
        ksh_codes = []

        # 1. 기본 KSH 패턴으로 모든 코드 추출
        matches = re.findall(r"KSH\d{10}", text)
        ksh_codes.extend(matches)

        # 2. 혹시 남아있을 수 있는 레거시 패턴도 처리 (하위 호환성)
        legacy_patterns = [
            r"▼0(KSH\d{10})▲",  # 포맷된 KSH 패턴
            r"nlk:(KSH\d{10})",  # 네임스페이스 포함 패턴 (만약 누락된 것이 있다면)
        ]
    ...
    def add_ksh_labeled_column(self):
        ...
        ※ 이어하기(Resume): 이미 처리된 행(= ksh_labeled가 비어있지 않은 행)은 건너뛰고,
          아직 비어있는 행만 선택하여 처리합니다.
        """
        ...
        try:
            with sqlite3.connect(self.mapping_db_path) as conn:
                ...
                # -------------------
                # ✅ 추가: 시작 지연 원인 제거용 '필수 인덱스' 선생성
                #  - ksh 필터( IS NOT NULL / != '' ) 가속
                cursor.execute(
                    "CREATE INDEX IF NOT EXISTS idx_mapping_ksh ON mapping_data(ksh);"
                )
                #  - ksh_labeled NULL/빈값 판별 가속 (IS NULL / = '')
                cursor.execute(
                    "CREATE INDEX IF NOT EXISTS idx_mapping_ksh_labeled ON mapping_data(ksh_labeled);"
                )
                #  - id 정렬 가속(없다면)
                cursor.execute(
                    "CREATE INDEX IF NOT EXISTS idx_mapping_id ON mapping_data(id);"
                )
                conn.commit()
                # -------------------

                # 2. 전체/대상 레코드 수 확인 (인덱스 생성 후 수행)
                cursor.execute(
                ...
                logger.info(
                    f"전체 KSH 보유: {total_records:,}개 / 처리 대상(미처리): {pending_records:,}개"
                )
                # -------------------

                if pending_records == 0:
                    logger.info(
                        "처리할 대상이 없습니다. 이미 모든 레코드가 매핑되었습니다."
                    )
                    return
                ...
                # ✅ tqdm 진행률 표시 추가
                pbar = tqdm(total=pending_records, desc="KSH 레이블 매핑", unit="row")
                # -------------------
        ...
        logger.info(f"처리 결과: 총 {processed:,}개 처리, {updated:,}개 업데이트")
    ...
    def _extract_korean_only(self, labeled_text: str) -> str:
        ...
        for item in items:
            ...
            # -------------------
            # 모든 공백 제거 (내부 공백 포함)
            korean_part = re.sub(r"\s+", "", korean_part)
            # -------------------
            if korean_part:
